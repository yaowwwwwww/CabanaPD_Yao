#!/bin/bash
# ============================================================
# Scan: JC_B for fixed JC_A=2e9, vin in [200, 600] m/s
...
if [ -n "${RUNS_ROOT_OVERRIDE:-}" ]; then
  ...
  RUNS_ROOT="${BASE_DIR}/runs_jcc_B_scan_A2e9_v200_600_${DATE_TAG}"
  mkdir -p "${RUNS_ROOT}"
  SUMMARY_FILE="${RUNS_ROOT}/summary_cor_hmax_recomputed_${DATE_TAG_SHORT}.txt"
fi
...
JC_B_LIST=(0 1e7 1e8 1e9 2e9 3e9 4e9)

{
  echo "==== Scan: JC_B with fixed JC_A=2e9 and vin={200..600} (timestamp = ${DATE_TAG}) ===="
  date
  echo "Fixed LJ: alpha=${LJ_ALPHA_FIXED}, beta=${LJ_BETA_FIXED}"
  echo "Fixed JC: A=${JC_A_FIXED}, n=${JC_N_FIXED}, C=${JC_C_FIXED}"
...
for BALL_VIN in "${BALL_VIN_LIST[@]}"; do
  for JC_B in "${JC_B_LIST[@]}"; do
    CASE_NAME="vin_${BALL_VIN}_alpha_${LJ_ALPHA_FIXED}_beta_${LJ_BETA_FIXED}_A_${JC_A_FIXED}_B_${JC_B}_C_${JC_C_FIXED}"
    CASE_TAG="run_vin_${BALL_VIN}_alpha_${LJ_ALPHA_FIXED}_beta_${LJ_BETA_FIXED}_A_${JC_A_FIXED}_B_${JC_B}_C_${JC_C_FIXED}"
    CASE_DIR="${RUNS_ROOT}/${CASE_TAG}"

    if grep -q "^${CASE_NAME}[[:space:]]" "${SUMMARY_FILE}" 2>/dev/null; then
      echo "⏭️  Skip existing case in summary: ${CASE_NAME}"
      continue
    fi

    echo "-----------------------------------------------------"
    echo "🚀 Running case: V_in=${BALL_VIN}, A=${JC_A_FIXED}, B=${JC_B}, C=${JC_C_FIXED}"
    ...
    if [ $CABANA_STATUS -ne 0 ]; then
      ...
      printf "vin_%s_alpha_%s_beta_%s_A_%s_B_%s_C_%s  NaN  NaN  NaN  NaN  NaN  -1  NaN  NaN  NaN\n" \
        "$BALL_VIN" "$LJ_ALPHA_FIXED" "$LJ_BETA_FIXED" "$JC_A_FIXED" "$JC_B" "$JC_C_FIXED" >> "${SUMMARY_FILE}"
      continue
    fi
