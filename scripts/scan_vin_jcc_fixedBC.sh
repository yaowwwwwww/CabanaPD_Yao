#!/bin/bash
# ============================================================
# Scan: impact velocity vin
# Fixed: LJ params, JC A/B/C/n, CZM params
# ============================================================
...
RUNS_ROOT="${BASE_DIR}/runs_jcc_vin_scan_${DATE_TAG}"
mkdir -p "${RUNS_ROOT}"
...
# ========= Scan params =========
BALL_VIN_LIST=(200 250 300 350 400 450 500 550 600 650 700)

{
  echo "==== Scan: vin (timestamp = ${DATE_TAG}) ===="
  date
...
for BALL_VIN in "${BALL_VIN_LIST[@]}"; do
  echo "-----------------------------------------------------"
  echo "🚀 Running case: V_in=${BALL_VIN}, A=${JC_A_FIXED}, B=${JC_B_FIXED}, C=${JC_C_FIXED}"
  ...
  if [ $CABANA_STATUS -ne 0 ]; then
    ...
    printf "vin_%s_alpha_%s_beta_%s_A_%s_B_%s_C_%s  NaN  NaN  NaN  NaN  NaN  -1  NaN  NaN  NaN\n" \
      "$BALL_VIN" "$LJ_ALPHA_FIXED" "$LJ_BETA_FIXED" "$JC_A_FIXED" "$JC_B_FIXED" "$JC_C_FIXED" >> "${SUMMARY_FILE}"
    continue
  fi
  ...
  CASE_TAG="run_vin_${BALL_VIN}_alpha_${LJ_ALPHA_FIXED}_beta_${LJ_BETA_FIXED}_A_${JC_A_FIXED}_B_${JC_B_FIXED}_C_${JC_C_FIXED}"
  ...
  if ! run_recompute "${CASE_DIR}"; then
    echo "⚠️ Octave recompute failed for vin_${BALL_VIN}_alpha_${LJ_ALPHA_FIXED}_beta_${LJ_BETA_FIXED}_A_${JC_A_FIXED}_B_${JC_B_FIXED}_C_${JC_C_FIXED}"
    printf "vin_%s_alpha_%s_beta_%s_A_%s_B_%s_C_%s  NaN  NaN  NaN  NaN  NaN  -1  NaN  NaN  NaN\n" \
      "$BALL_VIN" "$LJ_ALPHA_FIXED" "$LJ_BETA_FIXED" "$JC_A_FIXED" "$JC_B_FIXED" "$JC_C_FIXED" >> "${SUMMARY_FILE}"
    continue
  fi
